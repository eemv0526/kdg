...
//  전체 조회 : 
  @GetMapping("/comic")
  public String selectComicList(@ModelAttribute("searchVO") Criteria searchVO, Model model) throws Exception {
//          TODO: 0) 페이징 변수에 설정 : 
     searchVO.setPageUnit(8); // 1페이지당 화면에 보이는 개수
...
//     TODO: 3) FileDB 테이블의 총개수(서비스 객체의 함수를 실행) : 페이지 객체 저장
     int totCnt = comicService.selectComicListTotCnt(searchVO);
...
//추가 페이지 열기 :
@GetMapping("/comicadd")
public String createComicView() {
   return "comic/comicadd";
}

//저장 버튼 클릭시 실행 :
@PostMapping("/comic/add")
public String createComic(@RequestParam(defaultValue = "") String comicTitle,
                  @RequestParam(defaultValue = "") String comicContent,
                  @RequestParam(required = false) MultipartFile image,
                  @RequestParam(defaultValue = "") String comicDescription,
                  @RequestParam(defaultValue = "") String comicGenre
      ) throws Exception {
	
	

//   0) 생성자 실행 : 
   ComicVO comicVO = new ComicVO(comicTitle, comicContent, image.getBytes(),comicDescription, comicGenre);
//   1) fileDbService 의 insert(객체) 함수 실행
   comicService.insertComic(comicVO);
   return "redirect:/comic"; // 전체조회 강제이동
}

  
//상세조회 + 첨부파일 전송(jsp) : byte[] 형태로 전송 : @ResponseBody
@GetMapping("/comic/comic/{uuid}")
@ResponseBody
...
// 2) 첨부파일 jsp 전송 : 규격대로 전송(코딩 보냄)
   ...
   headers.setContentDispositionFormData("attachment", comicVO.getUuid()); // 첨부파일(문서형태)
   headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);              // 첨부파일2(문서형태)

//  첨부파일 전송 + OK 신호 보냄
   return new ResponseEntity<byte[]>(comicVO.getComicData(), headers, HttpStatus.OK);
}
...
public String deleteComic(@RequestParam(defaultValue = "") String uuid) throws Exception {
   log.info("테스트 : " + uuid);
   comicService.deleteComic(uuid);
   return "redirect:/comic"; // 전체조회 강제이동

}
 
//상세조회 페이지 열기 :
@GetMapping("/comic/update")
public String updateupdateComicView(@RequestParam String uuid, Model model) 
          throws Exception {

 ComicVO comicVO = comicService.selectComic(uuid);
// 2) 모델에 담아 Jsp 로 전송
 model.addAttribute("comicVO", comicVO);
 return "comic/comicupdate";
}

@PostMapping("/comic/update/update")
public String updateLnv(@RequestParam String uuid, 
                         @ModelAttribute ComicVO comicVO, 
                         @RequestParam(required = false) MultipartFile image) throws Exception {

    // 이미지를 수정할 때, 새 이미지가 업로드되었는지 확인
    if (image != null && !image.isEmpty()) {
        // 새 이미지 파일을 DB에 저장
        comicVO.setComicData(image.getBytes());  // 이미지 데이터 저장
        // 수정된 이미지를 DB에 저장할 URL 설정
        comicVO.setComicUrl("http://localhost:8000/kwon/comic/comic/" + uuid);  // DB에 저장될 URL
    }

 
    comicService.updateComic(comicVO); // DB 업데이트

    return "redirect:/comic";  // 수정 완료 후 전체 목록 페이지로 리디렉션
}
