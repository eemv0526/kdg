...
.card-genre, .card-title, .card-text {
    ...
    <script type="text/javascript" defer="defer">
        ...
        function fn_egov_selectList() {
            ...
            document.listForm.action = "${pageContext.request.contextPath}/comic";
        ...
        function fn_add_list() {
            window.location.href = "${pageContext.request.contextPath}/comicadd";
        }
...
<body>
    ...
    <div class="container">
        ...
        <div class="text-center">
            ...
            <button id="fantasy-btn" class="btn genre-btn btn-info" onclick="filterGenre('모험')">모험</button>
            <button id="action-btn" class="btn genre-btn btn-danger" onclick="filterGenre('액션')">액션</button>
            <button id="mystery-btn" class="btn genre-btn btn-warning" onclick="filterGenre('요리')">요리</button>
            <button id="thriller-btn" class="btn genre-btn btn-dark" onclick="filterGenre('배틀')">배틀</button>
            <button id="romance-btn" class="btn genre-btn" style="background-color: #ff69b4; color: white;" onclick="filterGenre('로맨스')">로맨스</button>
            <button id="romance-btn" class="btn genre-btn" style="background-color: #000080; color: white;" onclick="filterGenre('스포츠')">스포츠</button>
            <button id="romance-btn" class="btn genre-btn" style="background-color: #87ceeb; color: white;" onclick="filterGenre('판타지')">판타지</button>
        </div>
        ...
        <form id="listForm" name="listForm" method="get">
            ...
            <div class="row">
                <c:forEach var="data" items="${comics}">
                    <div class="col-3">
                        <div class="card" onclick="location.href='${pageContext.request.contextPath}/comic/detail?uuid=${data.uuid}'">
                            <img src="<c:out value="${data.comicUrl}" />" class="card-img-top" alt="도서 이미지">
                            <div class="card-body">
                                <span class="card-genre"><c:out value="${data.comicGenre}" /></span>
                                <h5 class="card-title"><c:out value="${data.comicTitle}" /></h5>
                                <p class="card-text"><c:out value="${data.comicContent}" /></p>
                                ...
                                <c:if test="${infoVO.codeName == '관리자'}">
                                    <a href="${pageContext.request.contextPath}/comic/update?uuid=${data.uuid}" class="btn btn-warning">관리</a>
                                </c:if>
                            </div>
                        </div>
                    </div>
