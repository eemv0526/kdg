    ...
    .review-stars {
       ...
       <script type="text/javascript" defer="defer">
        ...
        function fn_delete() {
            ...
            document.detailForm.action = "${pageContext.request.contextPath}/cc/delete";
...
<body>
    ...
    <div class="container mt-5">
        <!-- 만화 상세 정보 출력 -->
        <c:if test="${not empty ComicVO}">
            <div class="card">
                <img src="<c:out value="${ComicVO.comicUrl}" />" class="card-img-top" alt="도서 이미지">
...
<h4>⭐ : ${formattedRating} 점</h4>
                ...
                <div class="card-body">
                    <h5 class="card-title"><c:out value="${ComicVO.comicTitle}" /></h5>
                    <p class="card-text summary"><strong>줄거리:</strong> <c:out value="${ComicVO.comicDescription}" /></p>
                    <p class="card-text genre"><strong>장르:</strong> <c:out value="${ComicVO.comicGenre}" /></p>
                    <p class="card-text"><c:out value="${ComicVO.comicContent}" /></p>
                    <a href="javascript:history.back()" class="btn btn-custom">뒤로가기</a>
                </div>
            </div>
        </c:if>
        <c:if test="${empty ComicVO}">
...
<div class="review-create-btn text-center">
    <a href="${pageContext.request.contextPath}/ccadd?uuid=${ComicVO.uuid}" class="btn">별점 및 리뷰 쓰기</a>
</div>
</div>
