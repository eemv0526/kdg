...
.card-genre, .card-title, .card-text {
    ...
    <script type="text/javascript" defer="defer">
        ...
        function fn_egov_selectList() {
            ...
            document.listForm.action = "${pageContext.request.contextPath}/list";
        ...
        function fn_add_list() {
            window.location.href = "${pageContext.request.contextPath}/listadd";
        }
...
<body>
    ...
    <div class="container">
        ...
        <div class="text-center">
            ...
            <button id="fantasy-btn" class="btn genre-btn btn-info" onclick="filterGenre('스릴러')">스릴러</button>
            <button id="action-btn" class="btn genre-btn btn-danger" onclick="filterGenre('액션')">액션</button>
            <button id="mystery-btn" class="btn genre-btn btn-warning" onclick="filterGenre('코미디')">코미디</button>
            <button id="thriller-btn" class="btn genre-btn btn-dark" onclick="filterGenre('전쟁')">전쟁</button>
            <button id="romance-btn" class="btn genre-btn" style="background-color: #ff69b4; color: white;" onclick="filterGenre('SF')">SF</button>
        ...
        <form id="listForm" name="listForm" method="get">
            ...
            <div class="row">
                <c:forEach var="data" items="${lists}">
                    <div class="col-3">
                        <div class="card" onclick="location.href='${pageContext.request.contextPath}/list/detail?uuid=${data.uuid}'">
                            <img src="<c:out value="${data.listUrl}" />" class="card-img-top" alt="도서 이미지">
                            <div class="card-body">
                                <span class="card-genre"><c:out value="${data.listGenre}" /></span>
                                <h5 class="card-title"><c:out value="${data.listTitle}" /></h5>
                                <p class="card-text"><c:out value="${data.listContent}" /></p>
                                               <div class="average-rating" style="text-align: center; margin-top: 20px;">

</div>     
            
                                <c:if test="${infoVO.codeName == '관리자'}">
                                    <a href="${pageContext.request.contextPath}/list/update?uuid=${data.uuid}" class="btn btn-warning">관리</a>
                                </c:if>
                            </div>
                        </div>
                    </div>
