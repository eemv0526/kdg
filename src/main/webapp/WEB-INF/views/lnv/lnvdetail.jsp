    ...
    .review-stars {
       ...
       <script type="text/javascript" defer="defer">
        ...
        function fn_delete() {
            ...
            document.detailForm.action = "${pageContext.request.contextPath}/lc/delete";
            // 2) submit() 실행
...
<body>
    ...
    <div class="container mt-5">
        <c:if test="${not empty LnvVO}">
            <div class="card">
                <img src="<c:out value="${LnvVO.lnvUrl}" />" class="card-img-top" alt="도서 이미지">
...
<h4>⭐ : ${formattedRating} 점</h4>
                ...
                <div class="card-body">
                    <h5 class="card-title"><c:out value="${LnvVO.lnvTitle}" /></h5>
                    <p class="card-text summary"><strong>줄거리:</strong> <c:out value="${LnvVO.lnvDescription}" /></p>
                    <p class="card-text genre"><strong>장르:</strong> <c:out value="${LnvVO.lnvGenre}" /></p>
                    <p class="card-text"><c:out value="${LnvVO.lnvContent}" /></p>
                    <a href="javascript:history.back()" class="btn btn-custom">뒤로가기</a>
                </div>
            </div>
        </c:if>
        <c:if test="${empty LnvVO}">
...
<div class="review-create-btn text-center">
    <a href="${pageContext.request.contextPath}/lcadd?uuid=${LnvVO.uuid}" class="btn">별점 및 리뷰 쓰기</a>
</div>
</div>
